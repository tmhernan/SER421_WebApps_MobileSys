...
function getName(s) {
    ...
    'Welcome ' + s + ',<br>You are now ready to play battleship. <br> Click the board to fire at a ship. Try to sink every battleship!';
...
function showRecord() {
    ...
    var list = document.getElementById('recordDetails');
    var recordList = JSON.parse(localStorage.getItem('record'));
    //openlist is false to start
    if (openList === false) {
        if(localStorage.getItem('record') === null){
            document.getElementById('recordDetails').innerHTML = "";
        }else{
            var htmlStr = '<ol>';
            console.log("in open list true")
            for (const element of recordList) {
                htmlStr += '<li>' + element.username + ' ' + element.type + '</li>';
            ...
            document.getElementById('recordDetails').innerHTML = htmlStr;
...
function clearRecord(){
    var response = confirm("Are you sure you want to clear the record?")
    if (response == true){
        localStorage.removeItem('record');
...
function restartGame() {
    ...
    if (localStorage.getItem('record') === null) {
        record = [];
        record.push(restarts);
        localStorage.setItem('record', JSON.stringify(record));
        console.log(restarts.username)
        console.log(restarts.type)
    } else {
        var recordRead = [];
        var oldRecord = JSON.parse(localStorage.getItem('record'));
        recordRead.push(oldRecord)
        recordRead.push(restarts);
        localStorage.setItem('record', JSON.stringify(recordRead));
        console.log(restarts.username)
        console.log(restarts.type)
    }
